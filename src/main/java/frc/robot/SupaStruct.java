...
public class SupaStruct {
    ...
    private boolean resetNavx, resetDrive, xbutton, ybutton, rbbutton, lbbutton, abutton, ltrigger, rtrigger, pov,
            povToggled, itsreal = false;
    ...
    public void updateTele() {
        ...
        if (Math.abs(xbox.getRawAxis(DriveInput.rcwY)) >= 0.1 || Math.abs(xbox.getRawAxis(DriveInput.rcwX)) >= 0.1) {
            ...
            rcw = rcwX / 5;
            povToggled = false;
            // !povToggled is so moving the stick disabled the auto rotate pov function
            // (like in video games, shooting a gun disables the ability to sprint)
        } else if (povToggled) {
        ...
        // else statements
        if (/* !ybutton&& */ !povToggled && /* !bbutton&& */ Math.abs(xbox.getRawAxis(DriveInput.rcwY)) < 0.1
